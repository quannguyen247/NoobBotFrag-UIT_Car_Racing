...
MAX_SPEED = 70 
MIN_SPEED = 33  # ĐÃ TĂNG THEO YÊU CẦU
MAX_ANGLE = 25
KP0 = 0.30      # ĐÃ TĂNG NHẸ: Tăng ổn định giữa làn
KI0 = 0.0015
KD0 = 0.25      
SPEED_DECAY = 5
Y_THRESHOLD = 92       
...
LOOK_AHEAD_DISTANCE_Y = 40 
MIN_LOOK_AHEAD_Y_RATIO = 0.1 
POLYFIT_DEGREE = 2        

RIGHT_MARGIN_BASE = 50      # ĐÃ GIẢM: Dịch chuyển cơ bản gần trung tâm hơn
MIN_MARGIN = 35             
MAX_MARGIN = 65             # ĐÃ GIẢM: Giới hạn dịch chuyển tối đa
CURVATURE_DECAY_FACTOR = 0.9 # ĐÃ TĂNG: Tăng tác động khi cua để tránh cán lề
...
def analyze_and_select_straight_lane(all_blobs_by_y, y_positions, width, height, last_poly):
    ...
    default_target_x = width // 2 + RIGHT_MARGIN_BASE 

    for y in y_positions:
        blobs = [b for b in all_blobs_by_y[y] if b['center'] > width // 2 - 50] 
        ...
        if is_fork_detected and len(blobs) >= 2:
            ...
            if best_blob:
                chosen_blob_center = best_blob['center']
                 ...
                 chosen_blob_center = min(b['center'] for b in blobs) 
...
if __name__ == "__main__":
    try:
        while True:
            ...
            cv2.imshow('binary_lane_raw', binary_lane) 

            height, width = binary_lane.shape[:2]
            
            v_state = state.get("speed", 33)
            ...
            if lane_centers:
                ...
                if len(lane_centers) >= POLYFIT_DEGREE + 1: 
                    ...
                    # DỊCH CHUYỂN ĐỘNG
                    margin_dynamic = RIGHT_MARGIN_BASE - (curvature * CURVATURE_DECAY_FACTOR)
                    margin_fixed = np.clip(margin_dynamic, MIN_MARGIN, MAX_MARGIN)

                    predicted_lane_center_at_lookahead = np.polyval(poly, y_look_ahead)
                    last_predicted_center = predicted_lane_center_at_lookahead 

                    car_ref_x = int(predicted_lane_center_at_lookahead - margin_fixed)
            ...
            print(f"Speed: {speed:.1f} | Angle: {angle:.1f}")
